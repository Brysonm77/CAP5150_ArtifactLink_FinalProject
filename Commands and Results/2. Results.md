...
* UFW implementation
  * Port 22 was diabled.
  * OpenSSH is no longer usable.
  * WG can still connect and file trasnfer (all information is passed through the tunnel in port 51280)
 * SSH Allow list
   * All clients were denied access to all blocked users through SSH
   * Clients were denied access even when trying to use WG to bypass 
* Packet capture
  * SSH direct
    * TCP protocol is obeserved
    * Port 22 is observed
    * Shows destination IP and source IP
    * Data is encrypted 
  * WG outer
    * UDP protocol is oberserved
    * Port 51280 is observed
    * No SSH metadata is visible
  * WG inner (NOTE: this is only gathered because it was conducted on the server. An attacker WOULD NOT have access to this infomration)
    * TCP protocol is obeserved
    * Port 22 is observed
